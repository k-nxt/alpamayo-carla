...
| モード | スクリプト | 概要 |
|--------|-----------|------|
| **Agent（closed loop）** | `run_agent_nxt.py` | Alpamayoの推論結果で車両を実際に制御する |
| **Observer（open loop）** | `run_observer_nxt.py` | CARLAオートパイロットが走行し、Alpamayoは観察・推論のみ |
...
# その他依存パッケージ
uv pip install pygame numpy scipy torch torchvision transformers
```

一部CARLAのpythonコードを利用していますので、あるバージョンからCARLA_HOMEの設定が必要となっています  
...
python run_agent_nxt.py --model nvidia/Alpamayo-R1-10B --frames 500 --map Town01
python run_agent_nxt.py --model nvidia/Alpamayo-R1-10B --frames 500 --map Town01 --record output.mp4
python run_agent_nxt.py --model nvidia/Alpamayo-R1-10B --dummy --frames 200
...
# 基本実行（ナビゲーション自動有効）
python run_agent_nxt.py --frames 500 --map Town03

# CFG ナビゲーション（ガイダンス強化）
python run_agent_nxt.py --cfg-nav --frames 500

# ナビゲーション無効（R1 相当の動作）
python run_agent_nxt.py --no-nav --frames 500
```
...
5. （Agent のみ）VehiclePIDControllerによる制御

### Agent モード制御の特徴
- VehiclePIDControllerによる制御
- COTからの状態遷移で加速・停止の判定
...
- 推論は **RTXPRO** で約 0.8秒/推論（`num-traj-samples=1`, `diffusion-steps=10`）
- サンプル数を増やすと時間が増加（`num-traj-samples=6`時に3.6秒）
