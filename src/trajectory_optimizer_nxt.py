"""
Trajectory optimizer for post-processing predicted waypoints.

Smooths and refines model-predicted trajectories using classical
optimization (scipy.optimize.minimize) with cost functions for:
  - Trajectory smoothness (curvature penalty)
  - Deviation from original trajectory
  - Vehicle comfort constraints (yaw rate, longitudinal acceleration, jerk)
  - Frenet-style retiming (optional waypoint redistribution)
"""
...
class VehicleConstraints:
    ...
    max_deviation: float = 2.0       # metres from original trajectory
    max_heading_change: float = 0.5236  # ~30 deg in radians
    max_speed: float = 15.0          # m/s
    max_accel: float = 5.0           # m/s²

    # Comfort limits (PDMS reference values)
...
# ---------------------------------------------------------------------------
# Helper: add heading column
# ---------------------------------------------------------------------------

def add_heading_to_trajectory(xy_trajectory: np.ndarray) -> np.ndarray:
    """
    Convert [N, 2] (x, y) → [N, 3] (x, y, heading).

    Heading is computed from consecutive position deltas relative to
    the ego origin (0, 0).
    """
...
class TrajectoryOptimizer:
    """
    Post-process model-predicted trajectories for smoothness and comfort.

    Uses scipy L-BFGS-B (fallback: SLSQP) to minimise a cost that
    balances smoothness, deviation from the original prediction, and
    soft vehicle-comfort penalties.
    """
    ...
    def __init__(
        ...
        """
        Args:
            smoothness_weight: Weight for trajectory curvature penalty.
            deviation_weight: Weight for deviation from the model output.
            comfort_weight: Weight for comfort-limit soft penalty.
            max_iterations: Maximum optimiser iterations.
            enable_frenet_retiming: Redistribute waypoints along the
                path (Frenet-style) before optimising.
            retime_alpha: Retiming strength in [0, 1].  Higher values
                front-load the waypoint spacing.
        """
    ...
    def optimize(
        ...
        """
        Optimise a trajectory for smoothness and comfort.

        Args:
            trajectory: [N, 3] (x, y, heading) in rig/ego frame.
            time_step: Time between consecutive waypoints (seconds).
            vehicle_constraints: Vehicle kinematic limits.  Defaults
                used if ``None``.
            retime_in_frenet: Override ``enable_frenet_retiming``.
            retime_alpha: Override ``retime_alpha``.

        Returns:
            ``OptimizationResult`` containing the optimised trajectory.
        """
        ...
        # Skip optimisation for very short trajectories (stop / crawl).
        # These have too few meaningful waypoints for the solver to help,
        # and they often cause convergence failures.
        arc_len = float(np.sum(np.linalg.norm(np.diff(trajectory[:, :2], axis=0), axis=1)))
        if arc_len < 3.0 or trajectory.shape[0] < 5:
            return OptimizationResult(
                trajectory=trajectory,
                success=True,
                message="Skipped (trajectory too short for optimization)",
            )
        ...
        # Frenet retiming as initial guess
        if use_retiming:
            try:
        ...
        # Build cost & bounds (constraints not needed for L-BFGS-B)
        ...
        try:
            # L-BFGS-B only — fast and respects bounds.
            # SLSQP fallback removed: it is slow and rarely helps for
            # the trajectory sizes we deal with (~40 waypoints).
            result = minimize(
                cost_fn,
                x0,
                method="L-BFGS-B",
            ...
            if result.success:
                ...
                # Even when L-BFGS-B doesn't fully converge, the partial
                # result is usually an improvement.  Accept it rather
                # than discarding all work.
                logger.debug("L-BFGS-B partial convergence: %s", result.message)
                return OptimizationResult(
                    trajectory=optimized,
                    success=True,
                    iterations=getattr(result, "nit", None),
                    final_cost=float(result.fun),
                    message=f"Partial: {result.message}",
                )
    ...
    def _create_cost_function(
        ...
        def cost_function(x: np.ndarray) -> float:
            traj = x.reshape(-1, 3)
            smoothness = self._compute_smoothness_cost(traj)
            deviation = float(np.sum((traj - original_trajectory) ** 2))
            comfort = self._compute_comfort_penalty(traj, time_step, constraints)
            return (
                self.smoothness_weight * smoothness
                + self.deviation_weight * deviation
    ...
    def _compute_comfort_penalty(
        ...
        """Soft penalty for exceeding comfort limits."""
    ...
    def _compute_kinematics(
        ...
        """Compute yaw_rate, yaw_acc, lon_accel, lon_jerk."""
    ...
    def _create_constraints(
        ...
        if enforce_comfort:
            def comfort_margins(z: np.ndarray) -> np.ndarray:
                traj = z.reshape(-1, 3)
                yr, ya, la, lj = self._compute_kinematics(traj, time_step)
                margins = [
                    constraints.max_abs_yaw_rate - np.max(np.abs(yr)),
                    constraints.max_abs_yaw_acc - np.max(np.abs(ya)),
                    constraints.max_lon_acc_pos - np.max(la),
                    np.min(la) - constraints.max_lon_acc_neg,
                    constraints.max_abs_lon_jerk - np.max(np.abs(lj)),
                ]
    ...
    def _polyline_arclen(
        self, pts: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Cumulative arc-length along a 2-D polyline."""
        if pts.shape[0] < 2:
            return (
    ...
    def _sample_polyline_by_s(
        ...
        """Interpolate a 2-D polyline at given arc-lengths."""
        N = pts.shape[0]
        if N == 0:
    ...
    def _retime_along_path(
        ...
        """
        Redistribute waypoints along the path (Frenet-style).

        Keeps endpoints fixed; applies an ease-out warp so that
        waypoints are front-loaded (closer spacing near the ego).
        """
